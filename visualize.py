...
def save_excel(matrix, param, file_name=''):
    depth = param[0]
    head = param[1]
    data_to_save = pd.DataFrame(matrix)
    if os.path.exists('/apdcephfs/private_cookxu/deit_info/{}.xlsx'.format(file_name)):
        wb = openpyxl.load_workbook('/apdcephfs/private_cookxu/deit_info/{}.xlsx'.format(file_name))
        writer = pd.ExcelWriter('/apdcephfs/private_cookxu/deit_info/{}.xlsx'.format(file_name),
                                engine='openpyxl')
        writer.book = wb
    else:
        writer = pd.ExcelWriter('/apdcephfs/private_cookxu/deit_info/{}.xlsx'.format(file_name))
    data_to_save.to_excel(writer, 'depth{}_head{}'.format(depth, head), float_format='%.5f', index=None)
    writer.save()
    writer.close()


def save_att2excel(attention_list, file_name='', cls=True):
    if cls:
        for depth in range(len(attention_list)):
            for head in range(attention_list[depth].mean(0).shape[0]):
                attention_matrix = attention_list[depth].mean(0)[head].cpu().numpy()
                save_excel(attention_matrix, [depth, head], file_name)
    else:
        for depth in range(len(attention_list)):
            for head in range(attention_list[depth].mean(0).shape[0]):
                attention_matrix = attention_list[depth].mean(0)[head].cpu().numpy()
                save_excel(attention_matrix[1:, 1:], [depth, head], file_name)
...
@torch.no_grad()
def visualize(data_loader, model, device, excel_filename, post_process):
    ...
    model.stage_wise_prune = False

    for images_raw_full, target_full in metric_logger.log_every(data_loader, 10, header):
        # ipdb.set_trace()
        # npimg = images.squeeze(0).cpu().permute(1, 2, 0).numpy().astype('uint8')
        # plt.imsave('./test_img/{}.jpg'.format(str(target.item())), npimg)
        # images = torch.full(images.shape, 0)
        ...
        for index in range(B):
            ...
            save_image_tensor(images_raw,
                              '../deit_info/test_img_prune2/label{}_seed{}_index{}.jpg'.format(str(target.item()),
                                                                                               int(args.seed), index))
            ...
            for k in vis_dict:
                ...
                # image_pruned_save = images_raw * vis_map
                viz = gen_visualization(images_raw, keep_indices)
                viz = torch.from_numpy(viz).permute(2, 0, 1)
                # viz = viz.astype(np.uint8)

                # plt.imsave('../deit_info/test_img_prune/label{}_seed{}_{}_index{}.jpg'.format(str(target.item()),
                #                                                                               int(args.seed), k, index),
                #            viz)
                viz = viz / 255

                save_image_tensor(viz,
                                  '../deit_info/test_img_prune2/label{}_seed{}_{}_index{}_{}.jpg'.format(
                                      str(target.item()),
        ...
        # ipdb.set_trace()
        # save_att2excel(attention_recoder, file_name=excel_filename, cls=True)
        # save_att2excel(attention_recoder, file_name='attention_matrix', cls=False)
...
def main(args):
    ...
    if args.eval:
        test_stats = visualize(data_loader_val, model, device, args.excel_filename, post_process=post_process)
